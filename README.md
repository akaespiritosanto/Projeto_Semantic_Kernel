...
Ollama endpoint (choose one approach):
...
Build locally:
- `docker build -t kbairesearch/development:semantickernel.1.0 .`

Run by passing container parameters (env vars):
- `docker run -it --rm --name semantickernel.1.0 -e LOCATIONS_DB_CONNECTION_STRING=\"Data Source=locations.db\" -e OLLAMA_ENDPOINT_DOCKER=http://host.docker.internal:11434 -e OLLAMA_MODEL=llama3.1:latest kbairesearch/development:semantickernel.1.0`
